...
class SessionHistoryTest(unittest.TestCase):
    def test_whitelist_projection_only_user_and_final_text(self) -> None:
        ...
        # D23 §7.1: a failed latest turn also injects a journal reminder.
        self.assertEqual(5, len(items))
        # low-priority journal reminder leads the projection
        self.assertIsInstance(items[0], UserMessage)
        self.assertIn("[session-memory-reminder]", items[0].content)
        self.assertIsInstance(items[1], UserMessage)
        self.assertEqual("read the repo", items[1].content)
        self.assertIsInstance(items[2], AssistantMessage)
        self.assertEqual("done reading", items[2].item.text)
        # failed turn: its user request plus the reconstructed echo
        self.assertIsInstance(items[3], UserMessage)
        self.assertEqual("change add()", items[3].content)
        self.assertIsInstance(items[4], AssistantMessage)
        self.assertIn("[reconstructed-turn-outcome]", items[4].item.text)
        self.assertIn("errors=d2:model_client_failed", items[4].item.text)
