...
- 2026-09-20（WP-A 完成 + test-output gate）：**F20 根因修复**——loop 批次裁剪到"最后一个锚点之后的组"（跨锚跨度必被 recorder 拒绝；run11 的触发组是 seed 里的 vacuous prior-assistant 组 + 会话 user 锚点），同步契约化 `_append_group` 夹具（组后不再放 user 尾巴）。**test-output gate（WP-C 前置切片）**：`run_test_profile` 模型可见回执只含结构化诊断（exit/outcome/时长/字节/截断标记 + policy 版本标注），stdout/stderr 正文留在 durable 记录供运维——敏感日志不进模型服务的主入口关闭；`output_policy.py` 为单一权威。回归：71/71 绿（d23 全套 + wiring + metering + output gate + recall + d16/d19）。E 组剩余：WP-1（身份/可见等级正式化）、WP-D 投影事件族、WP-G 外发、WP-H 实测报告（需预算）。

- 2026-09-20（收口）：**全量回归 1084 项 / 0 errors / 32 skipped（Docker daemon 掉线）/ 唯一失败 = 已知 D25 close-hang flaky**（full-regression-hardening.txt）。E 组已闭环：WP-2 计量门禁、WP-A 基线（F21 契约钉扎 + anchor-trim 修复）、test-output gate（WP-C 前置：敏感日志不进模型服务的主入口关闭）、recall_history 元数据检索（WP-E 切片）。**显式未完成状态**：WP-1 通用身份/可见等级正式化（test-output gate 覆盖测试面）、WP-D 投影事件族、WP-G 固定产物外发、WP-H 真实长任务参数报告（需余额）、F20 range_missing 分支根因（需真实模型轮形态复现，归档库已备）、F22 诊断、Mimosa 完整扫描（scanner_enobufs）。预算约 4 元保留。

- 2026-09-20（WP-1 收口）：**全量回归 1084+ 项 OK / 0 errors / 32 skipped（Docker daemon 掉线）**——MCP 元数据-only 门禁全量无回归。WP-1 闭合。
